...
proto.next = function() {
  ...
  if (self.entries.length) {
    ...
    if (self.replay && self.entries.length) {
      ...
      replayTimeout = setTimeout(self.next.bind(self), delay * 1000);
    }
    else {
      process.nextTick(self.next().bind());
    }
  }
  else {
    debug("no entries");
...
proto.keypress = function(ch, key) {
  ...
  switch(key.name) {
    ...
    case 's': // status
      ...
      status.push('last entry is ' + self.entries[self.entries.length - 1].timestamp);
